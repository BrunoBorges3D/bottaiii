  ...
  client.on("voiceStateUpdate", async (oldState, newState) => {
    try {
      ...
      if (oldState.channelId && !newState.channelId) {
        ...
        // Dapatkan channel ID dari database
        const joinToCreateId = await db.get(`${oldState.guild.id}.voiceChannel`);
        
        // Jika channel kosong dan bukan "Join to Create", hapus channel
        if (oldState.channel?.id !== joinToCreateId && oldState.channel?.members.size === 0) {
